...
<Steps>
  <Step title="Navigate to the `op-node` package:">
  ```bash
    cd ~/optimism/op-node
    ```
  </Step>

  <Step title="Inside of the folder `rollup/derive`, create a new file called `l1_burn_info.go`:">
  ```bash
    touch rollup/derive/l1_burn_info.go
    ```
  </Step>

  <Step title="Paste the following into `l1_burn_info.go`, and make sure to replace `YOUR_BURN_CONTRACT_HERE` with the address of the `L1Burn` contract you just deployed.">
  ```go
    package derive
    ...
    func L1BurnDeposit(rollupCfg *rollup.Config, l1ChainConfig *params.ChainConfig, sysCfg eth.SystemConfig, seqNumber uint64, block eth.BlockInfo, l2Timestamp uint64) (*types.DepositTx, error) {
        infoDat := L1BurnInfo{
    ...
    func L1BurnDepositBytes(rollupCfg *rollup.Config, l1ChainConfig *params.ChainConfig, sysCfg eth.SystemConfig, seqNumber uint64, l1Info eth.BlockInfo, l2Timestamp uint64) ([]byte, error) {
        dep, err := L1BurnDeposit(rollupCfg, l1ChainConfig, sysCfg, seqNumber, l1Info, l2Timestamp)
        if err != nil {
...
<Steps>
  ...
  <Step title="After those lines, add this code fragment:">
  ```go
    l1BurnTx, err := L1BurnDepositBytes(ba.rollupCfg, ba.l1ChainConfig, sysConfig, seqNumber, l1Info, nextL2Time)
    if err != nil {
  ...
  <Step title="Immediately following, change these lines:">
  ```go
